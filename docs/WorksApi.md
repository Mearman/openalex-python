...
with openalex_api.ApiClient(configuration) as api_client:
    ...
    api_instance = openalex_api.WorksApi(api_client)
...
# **get_works**
> WorksResponseSchema get_works(api_key=api_key, cursor=cursor, filter=filter, group_by=group_by, page=page, per_page=per_page, sample=sample, search=search, select=select, sort=sort, user_agent=user_agent, mailto=mailto)
...
with openalex_api.ApiClient(configuration) as api_client:
    ...
    try:
        # /works
        api_response = api_instance.get_works(api_key=api_key, cursor=cursor, filter=filter, group_by=group_by, page=page, per_page=per_page, sample=sample, search=search, select=select, sort=sort, user_agent=user_agent, mailto=mailto)
