...
# **get_publishers**
> PublishersResponseSchema get_publishers(api_key=api_key, cursor=cursor, filter=filter, group_by=group_by, page=page, per_page=per_page, sample=sample, search=search, select=select, sort=sort, user_agent=user_agent, mailto=mailto)
...
with openalex_api.ApiClient(configuration) as api_client:
    ...
    try:
        # /publishers
        api_response = api_instance.get_publishers(api_key=api_key, cursor=cursor, filter=filter, group_by=group_by, page=page, per_page=per_page, sample=sample, search=search, select=select, sort=sort, user_agent=user_agent, mailto=mailto)
